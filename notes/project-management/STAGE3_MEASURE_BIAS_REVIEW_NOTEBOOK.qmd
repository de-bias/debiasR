...
population_lm_structure <- validate_bias_residual_structure(
  coverage_df = simulated_coverage,
...
bind_rows(
  ...
  population_lm_structure$summary
) |>
  select(
    residual_type,
...
```{r}
population_lm_structure$population_lm
```
...
tibble(
  check = c(
    ...
    "population-only linear model summary returned",
  ...
  passed = c(
    ...
    all(c("population_slope", "r_squared") %in% names(bias_structure$population_lm)),
