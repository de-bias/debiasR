...
`debiasR::measure_bias()` expects the benchmark population column to be called `population` and the digital trace population column to be called `user_count`. These names are already used in the workshop coverage table; we also keep a standard `area` identifier for the later residual diagnostic.
